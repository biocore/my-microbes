...
def parse_recipients(recipients_f):
    ...
    Returns a dictionary mapping personal ID to a list of email addresses.
    ...
    Arguments:
        recipients_f - the recipients file (two-column TSV with personal ID and
            comma-separated email address(es))
    """
    results = {}
        ...
        if not _can_ignore(line):
            ...
            if len(fields) != 2:
                raise ValueError("Each line in the recipients file must "
                                 "contain exactly two fields separated by "
                                 "tabs.")
